...
source("plinkshack.R") #would run with old plinks.R
...
colnames(X) <- rep("beta", length(sd_X))
 return(list(data = rbinom(n,1,ao2(nu)$linkinv(rowSums(X)+rnorm(n, sd = sd_eps))), X = X))
}

ao2_family <- function(nu) binomial(link = ao2(nu))

fit_ao2 <- function(data_list){
  glmx.fit(model.matrix(data_list$data~data_list$X), data_list$data, family = ao2_family,
           xlink = "log", xstart = 0, xlower = 0, xupper = 5, hessian = FALSE)
}
...
do.sims <- function(nu, n, sd_X, type, sd_eps, repl = 1){
  ...
  for(i in 1:nrow(output)){
    ...
    for(j in 1:repl){
#    fff <- fit_ao2(generate_data(n,nu,sd_X, sd_eps,type))
      fff <- numeric()
      class(fff) <- "try-error"
      while(class(fff) == "try-error"){
      fff <- try(fit_ao2(generate_data(n,nu,sd_X, sd_eps,type)))
    ...
    save(output, file = "ao2_sim_incomplete.Robj")
  }
  output <- list(output = output, repl = repl, te = try_error_counter)
  save(output, file = "ao2_sim.Robj")
}
